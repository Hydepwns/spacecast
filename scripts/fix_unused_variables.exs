...
defmodule FixUnusedVariables do
  ...
  defp extract_unused_variables(output) do
    ...
    unused_vars =
      lines
      |> Enum.with_index()
      |> Enum.filter(fn {line, _i} -> String.contains?(line, "warning: variable ") and String.contains?(line, "is unused") end)
  ...
  defp fix_file_variables(file, vars) do
    if not File.exists?(file) do
      ...
      # Apply fixes
      updated_lines = Enum.reduce(sorted_vars, lines, fn var, acc ->
